...
import UIKit
import PermissionScopeContinue

class ViewController: UIViewController {
    let singlePscope = PermissionScopeContinue()
    let multiPscope = PermissionScopeContinue()
    let noUIPscope = PermissionScopeContinue()
...
//        multiPscope.addPermission(BluetoothPermission(), message: "We use this to drain your battery")
    ...
    func checkContacts() {
        switch PermissionScopeContinue().statusContacts() {
        case .unknown:
            // ask
            PermissionScopeContinue().requestContacts()
